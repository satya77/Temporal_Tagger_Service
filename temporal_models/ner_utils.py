...
class TokenClassfication:
    pass


class TokenClassificationTask(TokenClassfication):
    @staticmethod
...
class TokenClassificationWithDate(TokenClassfication):
    @staticmethod
    def read_examples_from_file(data_dir, mode: Union[Split, str]) -> List[InputExample]:
        raise NotImplementedError
    ...
    def convert_examples_to_features(
            ...
            max_seq_length: int,
            tokenizer: PreTrainedTokenizer,
            tokenizer_date: PreTrainedTokenizer,
            cls_token_at_end=False,
            cls_token="[CLS]",
            cls_token_segment_id=1,
            sep_token="[SEP]",
        ...
        for (ex_index, example) in enumerate(examples):
            ...
            for word, label in zip(example.words, example.labels):
                word_tokens = tokenizer.tokenize(word)
                date_tokens = tokenizer_date.tokenize(example.guid)
                # bert-base-multilingual-cased sometimes output "nothing ([]) when calling tokenize with just a space.
                if len(word_tokens) > 0:
                    tokens.extend(word_tokens)
                    # Use the real label id for the first token of the word, and padding ids for the remaining tokens
            ...
            input_ids = tokenizer.convert_tokens_to_ids(tokens)
            input_date_ids = tokenizer_date.convert_tokens_to_ids(date_tokens)
            # The mask has 1 for real tokens and 0 for padding tokens. Only real
            # tokens are attended to.
            input_mask = [1 if mask_padding_with_zero else 0] * len(input_ids)
            ...
            if ex_index < 5:
                ...
                logger.info("guid: %s", example.guid)
                logger.info("tokens: %s", " ".join([str(x) for x in tokens]))
                logger.info("date: %s", " ".join([str(x) for x in date_tokens]))
                logger.info("input_ids: %s", " ".join([str(x) for x in input_ids]))
                logger.info("input_date_ids: %s", " ".join([str(x) for x in input_date_ids]))
                logger.info("input_mask: %s", " ".join([str(x) for x in input_mask]))
                logger.info("segment_ids: %s", " ".join([str(x) for x in segment_ids]))
                logger.info("label_ids: %s", " ".join([str(x) for x in label_ids]))
            ...
            features.append(
                InputFeatures(input_date_ids=input_date_ids,
